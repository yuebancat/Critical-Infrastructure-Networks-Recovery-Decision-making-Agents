...
input_file = 'Task_Plan_gpt-4o_question4UI.xlsx'  # Update the file path
df = pd.read_excel(input_file)

# Initialize a new DataFrame to store responses and logs
responses_logs = pd.DataFrame(columns=["query", "Agent_3_Response", "Agent_3_Logs"])

# Output file path
output_file = 'Task_Answer_gpt-4o_question4UI.xlsx'
...
for index, row in df.iterrows():
    ...
    try:
        ...
        print(f"Error: Tool {e} not found. Skipping this query.")
        continue  # Skip to the next query if the tool is not found
    ...
    # Capture Agent 3's response and logs
    response_3, logs_3 = None, ""
    try:
        ...
        continue  # Skip this task and continue with the next query
